...
export default function ProfilePage(): React.JSX.Element {
  ...
  const handleCopy = () => {
    ...
    // 1. Telegram WebApp native API — works in all Telegram versions including WebView
    const tg = (window as any).Telegram?.WebApp;
    if (typeof tg?.writeTextToClipboard === 'function') {
      try {
        tg.writeTextToClipboard(link, (success: boolean) => { if (success !== false) ok(); });
        return;
      } catch { /* fallthrough */ }
    }
    ...
    // 3. execCommand fallback (works in Telegram WebView where userSelect:none is set globally)
  ...
  function execCommandCopy(text: string, onSuccess: () => void) {
    ...
    // Override global userSelect:none set by Telegram WebApp hook
      ...
      >
        ...
        <div className="mt-4 flex items-end justify-between">
          ...
          <div className="text-right">
            ...
            <p className="mt-0.5 text-xl font-bold text-white/70">
